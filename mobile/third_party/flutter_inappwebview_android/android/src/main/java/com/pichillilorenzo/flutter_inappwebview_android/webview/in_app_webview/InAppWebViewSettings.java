...
public class InAppWebViewSettings implements ISettings<InAppWebViewInterface> {
  ...
  public Map<String, Object> getRealSettings(@NonNull InAppWebViewInterface inAppWebView) {
    ...
    if (inAppWebView instanceof InAppWebView) {
      ...
      // HAOHAO PATCH (haohaoxuexi.tech): all WebSettingsCompat getters below are wrapped
      // in try/catch. On some OEM ROMs the system wraps android.webkit.WebSettings, which
      // breaks the androidx.webkit boundary-interface glue with a ClassCastException
      // ("android.webkit.WebSettingsWrapper cannot be cast to
      //  com.android.webview.chromium.ContentSettingsAdapter"). Reading a setting must
      // never crash — on failure the entry is simply omitted or read via the framework.
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.SAFE_BROWSING_ENABLE)) {
        try {
          ...
          Log.e(LOG_TAG, "WebSettingsCompat.getSafeBrowsingEnabled failed, skipping", e);
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.DISABLED_ACTION_MODE_MENU_ITEMS)) {
        try {
          ...
          Log.e(LOG_TAG, "WebSettingsCompat.getDisabledActionModeMenuItems failed, skipping", e);
        }
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.FORCE_DARK)) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.getForceDark failed, skipping", e);
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.FORCE_DARK_STRATEGY)) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.getForceDarkStrategy failed, skipping", e);
        }
      }
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.OFF_SCREEN_PRERASTER)) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.getOffscreenPreRaster failed, skipping", e);
      ...
      if (WebViewFeature.isFeatureSupported(WebViewFeature.ALGORITHMIC_DARKENING) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.isAlgorithmicDarkeningAllowed failed, skipping", e);
        }
      }
      if (WebViewFeature.isFeatureSupported(WebViewFeature.ENTERPRISE_AUTHENTICATION_APP_LINK_POLICY)) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.getEnterpriseAuthenticationAppLinkPolicyEnabled failed, skipping", e);
        }
      }
      if (WebViewFeature.isFeatureSupported(WebViewFeature.REQUESTED_WITH_HEADER_ALLOW_LIST)) {
        try {
          ...
          // HAOHAO PATCH: see the comment above WebViewFeature.SAFE_BROWSING_ENABLE.
          Log.e(LOG_TAG, "WebSettingsCompat.getRequestedWithHeaderOriginAllowList failed, skipping", e);
        }
      }
    }
    return realSettings;
