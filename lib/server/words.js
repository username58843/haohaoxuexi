...
function ensureLoaded() {
  ...
  }
  {
    ...
    const words = loadPackFile(HSK79_ID, { hskLevel: 7 })
    if (words.length > 0) {
      packs.set(HSK79_ID, words)
      registry.push({ id: HSK79_ID, title: 'HSK 7–9', group: 'hsk', count: words.length })
    }
  }
...
export function getHskLevelSizes() {
  ...
  sizes[7] = getPackWords(HSK79_ID)?.length || 0
  return sizes
}
