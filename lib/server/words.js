...
// The combined HSK 3.0 band 7–9 pack (hsk: 7 internally).
const HSK79_ID = 'hsk7-9'
...
function normalizeEntry(raw, { hskLevel = null } = {}) {
  ...
  if (typeof raw.idPinyin === 'string' && raw.idPinyin.trim()) word.idPinyin = stripBom(raw.idPinyin)
  if (Number.isInteger(raw.sense) && raw.sense >= 2 && raw.sense <= 9) word.sense = raw.sense
  if (Number.isInteger(raw.sourceNumber)) word.sourceNumber = raw.sourceNumber
  for (const key of ['sourceLabel', 'sourceLevels', 'partOfSpeech']) {
    if (typeof raw[key] === 'string') word[key] = raw[key]
  }
...
function loadPackFile(id, opts) {
  const file = path.join(WORDS_DIR, opts?.legacy && id.startsWith('hsk') ? 'legacy' : '', `${id}.json`)
  try {
...
function ensureLoaded({ legacy = false } = {}) {
  if (caches.has(legacy)) return caches.get(legacy)
  const packs = new Map()
  const registry = []

  for (const level of HSK_LEVELS) {
    const id = `hsk${level}`
    const words = loadPackFile(id, { hskLevel: level, legacy })
    packs.set(id, words)
    registry.push({ id, title: `${legacy ? 'HSK' : 'HSK 3.0 ·'} ${level}`, group: 'hsk', count: words.length })
  }
  {
    // HSK 3.0 band 7–9 — one combined official list (七–九级), hsk = 7.
    const words = loadPackFile(HSK79_ID, { hskLevel: 7, legacy })
    if (words.length > 0) {
      packs.set(HSK79_ID, words)
      registry.push({ id: HSK79_ID, title: legacy ? 'HSK 7–9' : 'HSK 3.0 · 7–9', group: 'hsk', count: words.length })
    }
  }
  ...
  // Search index over HSK packs only (the canonical lexicon, incl. 7–9).
  const norm = (s) => stripBom(s).toLowerCase()
  const hskPackIds = registry.filter((p) => p.group === 'hsk').map((p) => p.id)
  const searchIndex = hskPackIds.flatMap((packId) =>
    packs.get(packId).map((word) => ({
      word,
  ...
  const wordLevels = new Map(searchIndex.map(({ word }) => [word.id, word.hsk]))
  const cache = { packs, registry, searchIndex, wordLevels }
  caches.set(legacy, cache)
  return cache
}

export function getPacks(options) {
  return ensureLoaded(options).registry
}

export function getPackWords(id, options) {
  if (typeof id !== 'string') return null
  return ensureLoaded(options).packs.get(id) || null
}

export function getHskLevelSizes(options) {
  const sizes = {}
  for (const level of HSK_LEVELS) {
    sizes[level] = getPackWords(`hsk${level}`, options)?.length || 0
  }
  // 7 = the combined band 7–9 pack.
  sizes[7] = getPackWords(HSK79_ID, options)?.length || 0
  return sizes
}

export function getWordLevel(id, options) {
  return ensureLoaded(options).wordLevels.get(id) || null
}

export function searchWords({ q = '', level = null, page = 1, limit = 40, legacy = false } = {}) {
  const { searchIndex } = ensureLoaded({ legacy })
  const query = stripBom(q).toLowerCase()
  // 7 = the combined band 7–9.
  const lvl = level && [...HSK_LEVELS, 7].includes(Number(level)) ? Number(level) : null
...
export async function getPacksResolved(options) {
  const overrides = await getPackOverrides()
  if (overrides.size === 0) return getPacks(options)
  return getPacks(options).map((p) => {
...
export async function getPackWordsResolved(id, options) {
  const base = getPackWords(id, options)
