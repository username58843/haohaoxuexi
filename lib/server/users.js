...
// Documented security target (docs/ARCHITECTURE.md §3). Existing hashes keep
// their original cost; new/changed passwords use this.
const BCRYPT_COST = 12

export const DEFAULT_SETTINGS = {
  themeColor: 'jade',
  // 'system' — follow the OS light/dark preference on every launch. Users can
  // still pin an explicit dark/light theme in settings.
  theme: 'system',
  language: 'en',
  dailyGoal: 20,
  alwaysShowPinyin: false,
  alwaysShowTranslation: false,
  // Quiz audio: pronounce the prompt after every correct answer. Opt-in —
  // unexpected sound is worse than no sound, so new accounts start with it off
  // and the choice then follows the account across web and Android.
  quizSpeakOnCorrect: false,
}
...
export async function createUser({ email, password, name, emailVerified = false, settings = {} }) {
  const users = await getCollection('users')
  const now = new Date()
  const doc = {
    email,
 ...
 *
 * schemaVersion 3: theme default changed from 'dark' to 'system' (follow the
 * OS). 'dark' was the implicit default nobody chose, so stored 'dark' values
 * are migrated to 'system' once; users who *want* a fixed theme simply pick
 * it again in settings (the choice then sticks — v3 never re-runs).
 */
export async function migrateLegacyUser(doc) {
  if (!doc || doc.schemaVersion >= 3) return doc
  if (doc.schemaVersion === 2) return migrateThemeToSystem(doc)
  ...
  return migrateThemeToSystem(migrated)
}

/** schemaVersion 2 → 3: legacy default theme 'dark' becomes 'system'. */
async function migrateThemeToSystem(doc) {
  const users = await getCollection('users')
  const set = { schemaVersion: 3, updatedAt: new Date() }
  const hadLegacyDark = doc.settings?.theme === 'dark'
  if (hadLegacyDark) set['settings.theme'] = 'system'
  await users.updateOne({ _id: doc._id, schemaVersion: { $lt: 3 } }, { $set: set })
  const migrated = { ...doc, schemaVersion: 3 }
  if (hadLegacyDark) migrated.settings = { ...doc.settings, theme: 'system' }
  return migrated
}

/** Full account erasure (Play Store requirement). */
export async function deleteUserCompletely(userId) {
  const [users, decks, srs, logs, feedback, audit, known, notes, notebooks, bookProgress] =
    await Promise.all([
      getCollection('users'),
      getCollection('decks'),
      ...
      getCollection('known_words'),
      getCollection('notes'),
      getCollection('notebooks'),
      getCollection('book_progress'),
    ])
  await Promise.all([
    decks.deleteMany({ userId }),
    srs.deleteMany({ userId }),
    logs.deleteMany({ userId }),
    // The word-map "known" set is per-account data too — leaving it behind
    // orphaned the document (and left erasure incomplete).
    known.deleteMany({ userId }),
    // Notes, notebooks and reading progress are per-account data as well.
    notes.deleteMany({ userId }),
    notebooks.deleteMany({ userId }),
    bookProgress.deleteMany({ userId }),
