...
import { getPackWordsResolved, getHskLevelSizes, getWordLevel } from './words'
 ...
 * requested packs. `limit` 0 (the default UX since the session-size picker was
 * removed) means "everything", hard-capped at MAX_QUEUE to bound the payload.
 */
const MAX_QUEUE = 500

export async function getQueue({ userId, packs = [], limit = 0, legacy = false }) {
  const cards = await getCollection('srs_cards')
  const now = new Date()
  const safeLimit = limit > 0 ? Math.min(MAX_QUEUE, limit) : MAX_QUEUE
  ...
  if (queue.length < safeLimit && packs.length > 0) {
    ...
    outer: for (const packId of packs) {
      const words = (await getPackWordsResolved(packId, { legacy })) || []
...
export async function getSummary({ userId, tzOffset = 0, dailyGoal = 20, legacy = false }) {
  const cards = await getCollection('srs_cards')
  const logs = await getCollection('review_logs')
  const now = new Date()
  ...
  const [dueCount, byStateAgg, catalogCards, dayRows] = await Promise.all([
    cards.countDocuments({ userId, due: { $lte: now } }),
    cards.aggregate([
      { $match: { userId } },
      { $group: { _id: '$state', n: { $sum: 1 } } },
    ]).toArray(),
    cards.find({ userId }).project({ wordId: 1, intervalDays: 1 }).toArray(),
  ...
  const sizes = getHskLevelSizes({ legacy })
  const byLevel = {}
  // 7 = the combined HSK 3.0 band 7-9 pack.
  for (let level = 1; level <= 7; level++) {
    byLevel[level] = { total: sizes[level], seen: 0, mature: 0 }
  }
  // Reclassify for display only. User snapshots and scheduling stay untouched.
  for (const card of catalogCards) {
    const level = getWordLevel(card.wordId, { legacy })
    if (!level) continue
    byLevel[level].seen++
    if (card.intervalDays >= MATURE_INTERVAL_DAYS) byLevel[level].mature++
  }
...
/**
 * Leech radar: the user's most-forgotten cards. A card qualifies once it has
 * lapsed (graded "Again" after graduating) at least `minLapses` times.
 * Worst-first ordering: most lapses, then lowest ease, then most recent.
 */
export async function getDifficult({ userId, limit = 20, minLapses = 2 }) {
  const cards = await getCollection('srs_cards')
  const filter = { userId, lapses: { $gte: minLapses } }
  const [docs, total] = await Promise.all([
    cards
      .find(filter)
      .sort({ lapses: -1, ease: 1, updatedAt: -1 })
      .limit(limit)
      .toArray(),
    cards.countDocuments(filter),
  ])
  return { items: docs.map(publicCard), total, minLapses }
}
