...
import { getPackWordsResolved, getHskLevelSizes } from './words'
...
export async function getQueue({ userId, packs = [], limit = 0 }) {
  const cards = await getCollection('srs_cards')
  const now = new Date()
  ...
  if (queue.length < safeLimit && packs.length > 0) {
    ...
    outer: for (const packId of packs) {
      const words = (await getPackWordsResolved(packId)) || []
...
export async function getSummary({ userId, tzOffset = 0, dailyGoal = 20 }) {
  ...
  const [dueCount, byStateAgg, byLevelAgg, dayRows] = await Promise.all([
    ...
    cards.aggregate([
      { $match: { userId, 'word.hsk': { $gte: 1, $lte: 7 } } },
      {
        $group: {
          _id: '$word.hsk',
          seen: { $sum: 1 },
