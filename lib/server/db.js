...
async function buildIndexes(db) {
  const spec = [
    ...
    ['known_words', [{ key: { userId: 1 }, unique: true }]],
    ['pack_overrides', [{ key: { packId: 1 }, unique: true }]],
    [
      'notes',
      [
        { key: { userId: 1, updatedAt: -1 } },
        { key: { userId: 1, notebookId: 1 } },
        { key: { userId: 1, trashed: 1, deletedAt: 1 } },
      ],
    ],
    ['notebooks', [{ key: { userId: 1, order: 1 } }]],
    ['book_progress', [{ key: { userId: 1, bookId: 1 }, unique: true }]],
    ['feedback', [{ key: { createdAt: -1 } }]],
    ['audit_logs', [{ key: { createdAt: -1 } }]],
    ['content', [{ key: { scope: 1, key: 1, lang: 1 }, unique: true }]],
    [
      'rate_limits',
      [
  ...
  // Drop the legacy content index that was defined on the wrong field name
  // (`type` instead of `scope`). The old index treated every document as
  // type=null and blocked writes with a duplicate-key error.
  try {
    const indexes = await db.collection('content').listIndexes().toArray()
    const stale = indexes.find((i) => i.name === 'type_1_key_1_lang_1')
    if (stale) {
      await db.collection('content').dropIndex('type_1_key_1_lang_1')
      console.log('Dropped stale content index: type_1_key_1_lang_1')
    }
  } catch (_) {
