...
export function makeWordId(word) {
  ...
  const sense = Number.isInteger(word?.sense) && word.sense >= 2 && word.sense <= 9 ? `~${word.sense}` : ''
  return `${simplified}·${pinyinKey(word?.idPinyin || word?.pinyin)}${sense}`
}
...
export function toWordSnapshot(word) {
  ...
  if (typeof word.idPinyin === 'string' && word.idPinyin.trim()) snap.idPinyin = clip(word.idPinyin).trim()
  if (Number.isInteger(word.sense) && word.sense >= 2 && word.sense <= 9) snap.sense = word.sense
