...
export const DEFAULTS = {
  // 'system' — the OS light/dark preference is detected on every visit (and
  // followed live). An explicit dark/light pick in settings still overrides.
  theme: 'system',
  themeColor: 'jade',
  language: 'en',
  dailyGoal: 20,
  ...
  // Quiz audio after each correct answer — opt-in (see lib/server/users.js).
  quizSpeakOnCorrect: false,
}

const LS_KEY = 'xue_settings_v2'
// One-time client marker for the dark→system theme migration (v3): 'dark' was
// the old implicit default, so a stored 'dark' without this marker is treated
// as "never chose a theme" and flipped to 'system'. Mirrors the server-side
// schemaVersion 3 migration in lib/server/users.js and the pre-paint boot
// script in pages/_document.js.
const THEME_MIGRATED_KEY = 'xue_theme_v3'

function migrateLegacyDarkTheme(raw) {
  if (typeof window === 'undefined') return raw
  try {
    if (localStorage.getItem(THEME_MIGRATED_KEY) === '1') return raw
    localStorage.setItem(THEME_MIGRATED_KEY, '1')
    if (raw && typeof raw === 'object' && raw.theme === 'dark') {
      const next = { ...raw, theme: 'system' }
...
function normalizeSettings(raw) {
  ...
  return {
    ...
    quizSpeakOnCorrect: !!s.quizSpeakOnCorrect,
  }
}
...
function readLocal() {
  ...
  try {
    const raw = migrateLegacyDarkTheme(JSON.parse(localStorage.getItem(LS_KEY) || 'null'))
...
export function SettingsProvider({ children }) {
  ...
  // After logout, let the next login pull server settings again. Guest
  // preferences persisted in localStorage are left untouched — browser
  // language detection applies only on a true first visit (see readLocal),
  // so a guest's explicit language choice survives reloads.
  useEffect(() => {
